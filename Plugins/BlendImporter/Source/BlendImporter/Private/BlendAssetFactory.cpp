...
FString UBlendImportOptions::ToString() const
{
    ...
    Data.Append(TEXT(";"));
    Data.Append(bMergeChildMeshes ? TEXT("true") : TEXT("false"));
    return Data;
}
...
bool UBlendImportOptions::FromString(const FString& Data)
{
    ...
    // Something may have corrupted the data, it can't be trusted to parse!
    if (nArraySize == 0 || nArraySize >= 4)
    {
        return false;
    }
    ...
    if (nArraySize >= 3)
    {
        bMergeChildMeshes = Params[2].ToBool();
    }
...
UObject* UBlendAssetFactory::FactoryCreateFile(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
    ...
    {
        FText FilenameSimple = FText::FromString(IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*Filename));
        ...
        ImportOptions->bMergeChildMeshes = ImportDialog->IsMergeChildMeshes();
    }
...
bool UBlendAssetFactory::RunScriptOnBlendFile(const FString& Filename, const FString& ScriptName, FString& Output)
{
    ...
    if (ProcessHandle.IsValid())
    {
        ...
        // HACK: Blender Python seems to have setup stdout to convert newlines to CRLF style newlines on
        // Windows and I couldn't find an elegant way to prevent that, so let's just convert them all here...
        #if PLATFORM_WINDOWS
        ...
        if (ReturnCode != 0 && !Settings->IsDebug())
        {
            UE_LOG(LogBlendImporter, Error, TEXT("Blender ran into a problem while executing, for more information, turn Debug Mode on in \"Project Settings > Blend File Importer\" and run it again."));
        }
    ...
    {
        UE_LOG(LogBlendImporter, Error, TEXT("There was an issue running Blender at \"%s\". Check the path is correct through \"Project Settings > Blend File Importer\"."), *BlenderExePath.FilePath);
        bResult = false;
    }
...
bool UBlendAssetFactory::BlendFileAnalyse(const FString& Filename, TArray<FString>& Collections, FString& MaterialWarnings, bool& IsPacked)
{
    ...
    const int ResultIndex = Output.Find("ANALYSE_RESULT_START");
    if (ResultIndex == INDEX_NONE)
    {
        return false;
    }
    
    Output.MidInline(ResultIndex);
    ...
    for (FString Line : OutputLines)
    {
        ...
        {
            case 'C':
                // UE_LOG(LogBlendImporter, Log, TEXT("Collections (%d): %s"), Params[1].Len(), *Params[1]);
                Params[1].LeftChopInline(1);
...
bool UBlendAssetFactory::BlendFileExport(const FString& Filename, const bool& Unpack, FString& OutputFilename)
{
    ...
    FPlatformMisc::SetEnvironmentVar(TEXT("UNREAL_IMPORTER_COMBINE_CHILD_MESHES"), ImportOptions->bMergeChildMeshes ? TEXT("true") : TEXT("false"));
    ...
    {
        UE_LOG(LogBlendImporter, Error, TEXT("A problem was encountered while exporting from Blender and no FBX data was created."));
        return false;
    }
