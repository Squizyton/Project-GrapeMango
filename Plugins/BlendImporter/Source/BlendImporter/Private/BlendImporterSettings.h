...
private:
	...
	UPROPERTY(Config, EditAnywhere, Category="External Tools", meta=(FilePathFilter = "Executables (*.exe, *.app)|*.exe;*.app", DisplayName = "Blender Executable Path"))
	...
	UPROPERTY(Config, EditAnywhere, Category="Options", meta=(DisplayName = "Unresponsive Warning Duration (s)", ClampMin = "0.0"))
	double UnresponsiveWarningDuration = 15.0;

	/** Runs Blender in debug mode, increasing debug output to identify problems. */
	UPROPERTY(Config, EditAnywhere, Category="Debug", meta=(DisplayName = "Blender - Debug mode"))
	bool bDebug = false;

	/** Run Blender in factory startup mode, resetting Blender to defaults when running (no addons, no startup scene, etc). Enable this is if you are having issues caused by awry addons. */
